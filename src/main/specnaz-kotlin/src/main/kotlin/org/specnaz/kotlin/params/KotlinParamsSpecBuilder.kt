...
class KotlinParamsSpecBuilder(val paramsSpecBuilder: ParamsSpecBuilder) : KotlinSpecBuilder(paramsSpecBuilder) {
    fun <P> should(description: String, testBody: TestClosureParams1<P>): ParamsExpected1<P> {
        return paramsSpecBuilder.should(description, testBody)
    }

    inline fun <reified T : Throwable, P> shouldThrow(
            description: String, testBody: TestClosureParams1<P>): ParamsExpectedException1<T, P> {
        return paramsSpecBuilder.shouldThrow(T::class.java, description, testBody)
    }
